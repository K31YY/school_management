...
class _AddTeacherState extends State<AddTeacher> {
  ...
  String _formatID(dynamic id) {
    ...
    // Converts 1 to 001, 10 to 010, etc.
  ...
  Future<void> _fetchUserId() async {
    ...
    if (id == null) {
      // Fallback: check if it was saved as a String
    ...
    if (id != null) {
      if (mounted) {
        setState(() {
          _currentUserId = id;
          // Set the text to the formatted version: UTB001
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      // --- MATCHING THE LARAVEL CONTROLLER KEYS EXACTLY ---
      request.fields['UserID'] =
          _currentUserId?.toString() ?? ""; // Changed from user_id
      ...
      request.fields['password'] =
          _passwordCtrl.text; // Changed from Password (lowercase)
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // This will print the exact validation error (like "The email has already been taken")
        print("Server Error: ${response.body}");
        EasyLoading.showError("Save Failed. Check if email exists.");
      }
    } catch (e) {
      EasyLoading.dismiss();
  ...
  // ... (Rest of your UI code remains exactly the same as your original)
